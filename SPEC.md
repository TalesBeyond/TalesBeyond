...
- **Audio (REQ-009):** bucket `table-audio` (public-read, host-only upload/delete scoped to a hosted table id, 10 MB object limit, MP3/WAV allow-list) holds Synced Table Audio. Guest DMs never upload audio: their files stay in the DM's browser and only the DM hears them. Cloud tables are capped at 50 MB of audio, enforced by a database trigger. Local demo mode has no audio.
